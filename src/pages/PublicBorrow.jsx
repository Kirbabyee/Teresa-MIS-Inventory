...
import { fetchInventoryCatalog, fetchInventoryItems, getTabTableConfig } from "@/lib/inventoryApi";
import { User, Package, CheckCircle, Search, Plus, Minus, Trash2 } from "lucide-react";
...
export default function PublicBorrow() {
  ...
  const [form, setForm] = useState({ name: "", studentId: "", role: "", expectedReturnAt: "", borrowDate: "" });
  ...
  const addToCart = useCallback((item, isCustom = false, forcedQuantity = null) => {
  ...
  // ── Load inventory items when entering Step 2 ────────────────────────────
  useEffect(() => {
    if (activeStep !== 2 || inventoryLoadedRef.current) return;
    ...
    const load = async () => {
      ...
      try {
        ...
        const tabTableNames = {};
        await Promise.all(
          catalog.map(async (tab) => {
            try {
              const config = await getTabTableConfig(tab.id);
              tabTableNames[tab.id] = config?.tableName || null;
            } catch {
              tabTableNames[tab.id] = null;
            }
          })
        );
        ...
        const queries = [];
        catalog.forEach((tab) => {
          const tableName = tabTableNames[tab.id];
  ...
  const validateStep = (step) => {
    ...
    if (step === 2) {
      if (borrowCart.length === 0 && customItems.length === 0 && selectedInventoryItems.length === 0)
        errs.items = "Add at least one item to borrow.";
    }
    setFormErrors(errs);
  ...
  const isStepValid = (step) => {
    ...
    if (step === 2) return borrowCart.length > 0 || customItems.length > 0 || selectedInventoryItems.length > 0;
    return true;
  };
  ...
  const addCustomItem = () => {
    ...
    const quantityValue = String(customItemForm.quantity || "").trim();
    ...
    // Add to both customItems (for submission) and borrowCart (for display)
    setCustomItems((c) => [...c, next]);
    const cartId = `custom-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`;
    setBorrowCart((prev) => [...prev, {
      ...next,
      cartId,
      isCustom: true,
      quantity: quantity,
  ...
  // ── Inventory item helpers (selectedInventoryItems legacy) ───────────────
  ...
  const updateInventoryItemQty = (itemId, qty) => {
  ...
  const confirmBorrow = async () => {
    ...
    try {
      const invItems = borrowCart.map((c) => ({
        label: c.label || getItemLabel(c),
        inventoryItemId: c.isCustom ? null : c.id,
        inventoryTabId: c.isCustom ? null : c.tabId,
        inventorySectionId: c.isCustom ? null : c.sectionId,
        inventoryTableName: c.isCustom ? "" : c.tableName,
        details: (c.details && c.details.length > 0)
          ? c.details
          : [
      ...
      setForm({ name: "", studentId: "", role: "", expectedReturnAt: "", borrowDate: "" });
      setCustomItems([]);
      setBorrowCart([]);
  ...
  const canAddCustom = customItemForm.name && String(customItemForm.quantity).trim();
          ...
          <div className="flex-1 overflow-y-auto px-6 py-5 sm:px-8 bg-white">
            ...
            {activeStep === 1 && (
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <Select value={form.role || ""} onValueChange={(val) => { setForm({ ...form, role: val }); setFormErrors((prev) => ({ ...prev, role: validateField("role", val) })); setFormError(""); }}>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <button type="button" onClick={() => setShowBorrowDatePicker((open) => !open)} className="flex w-full items-center gap-2.5 rounded-lg border border-input bg-white px-3 text-left text-sm shadow-sm transition hover:border-slate-300 h-9">
                          <CalendarIcon className="h-4 w-4 shrink-0 text-slate-400" />
                          <span className={cn("flex-1 truncate", form.borrowDate ? "text-slate-700 font-medium" : "text-slate-400")}>
                            {form.borrowDate
                          ...
                          <>
                            ...
                            <div className="fixed left-1/2 top-1/2 z-[201] w-[320px] -translate-x-1/2 -translate-y-1/2 animate-[calPopIn_200ms_ease-out] rounded-lg border-2 border-slate-200 bg-gradient-to-b from-white to-slate-50/90 shadow-[0_24px_80px_rgba(15,23,42,0.22)] ring-1 ring-white/60">
                              <div className="flex items-center gap-2.5 border-b border-slate-100 px-4 py-3">
                                ...
                                <button type="button" onClick={() => setShowBorrowDatePicker(false)} className="ml-auto rounded-md p-1 text-slate-400 hover:bg-slate-100 hover:text-slate-600"><Trash2 className="h-3.5 w-3.5" /></button>
                              </div>
                              ...
                              <div className="flex items-center justify-between gap-2 border-t border-slate-100 px-4 py-2.5">
                                <button type="button" onClick={() => setForm({ ...form, borrowDate: "" })} className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-xs text-slate-700 hover:bg-slate-100">Clear</button>
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <button type="button" onClick={() => setShowReturnDatePicker((open) => !open)} className="flex w-full items-center gap-2.5 rounded-lg border border-input bg-white px-3 text-left text-sm shadow-sm transition hover:border-slate-300 h-9">
                          <CalendarIcon className="h-4 w-4 shrink-0 text-slate-400" />
                          <span className={cn("flex-1 truncate", form.expectedReturnAt ? "text-slate-700 font-medium" : "text-slate-400")}>
                          ...
                          <>
                            ...
                            <div className="fixed left-1/2 top-1/2 z-[201] w-[320px] -translate-x-1/2 -translate-y-1/2 animate-[calPopIn_200ms_ease-out] rounded-lg border-2 border-slate-200 bg-gradient-to-b from-white to-slate-50/90 shadow-[0_24px_80px_rgba(15,23,42,0.22)] ring-1 ring-white/60">
                              <div className="flex items-center gap-2.5 border-b border-slate-100 px-4 py-3">
                                ...
                                <button type="button" onClick={() => setShowReturnDatePicker(false)} className="ml-auto rounded-md p-1 text-slate-400 hover:bg-slate-100 hover:text-slate-600"><Trash2 className="h-3.5 w-3.5" /></button>
                              </div>
                              <div className="px-3 pt-2">
                                <DayPicker
                                  className="rdp-sidebar-picker text-sm"
                                  ...
                                  footer={form.expectedReturnAt ? new Date(form.expectedReturnAt + "T00:00:00").toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }) : ""}
                                />
                              </div>
                              <div className="flex items-center justify-between gap-2 border-t border-slate-100 px-4 py-2.5">
            ...
            {activeStep === 2 && (
              <div className="space-y-4">
                          ...
                          <div key={cartId} className="grid grid-cols-[1fr_80px_90px_80px] gap-2 items-center border-b border-slate-100 px-3 py-2.5 hover:bg-slate-50 transition-colors">
                            ...
                            <div className="text-center">
                              <span className={cn(
                                ...
                                liveStock > 0 ? "bg-emerald-50 text-emerald-700 border border-emerald-200" : "bg-rose-50 text-rose-700 border border-rose-200"
                              )}>
                                {liveStock > 0 ? liveStock : "0"}
                              </span>
                            </div>
                            <div className="text-right">
                              ...
                              {liveStock > 0 && (
                                <Button
                                  ...
                                  onClick={() => {
                                    if (!alreadyInCart && liveStock > 0) {
                                      setQtyDialogItem(item);
                                      setQtyDialogValue(1);
                                    }
                                  }}
                                  disabled={alreadyInCart || liveStock <= 0}
                                  className={cn(
                                    "h-7 px-2.5 text-[11px] font-semibold mt-0.5",
                                    !alreadyInCart && liveStock > 0 && "bg-[#4a1111] hover:bg-[#5a1717]"
                                  )}
                                >
                ...
                {/* ── Quantity Dialog (exact copy from Borrowing.jsx) ──────────────── */}
                {qtyDialogItem && (
                  <>
                ...
                {/* ── Custom Item Form (exact copy from Borrowing.jsx) ──────────────── */}
                <div className="rounded-xl border border-dashed border-slate-300 bg-white p-4">
                  ...
                  <Button
                    ...
                    className={cn(
                      ...
                      canAddCustom ? "bg-[#4a1111] hover:bg-[#5a1717]" : "bg-slate-100 text-slate-400 cursor-not-allowed hover:bg-slate-100"
                    )}
                  >
                    + Add Custom Item
                  </Button>
                ...
                {/* ── Cart Summary with +/- controls (exact copy from Borrowing.jsx) ── */}
                {borrowCart.length > 0 && (
                    ...
                    <div className="divide-y divide-slate-100 max-h-[180px] overflow-y-auto">
                      {borrowCart.map((item) => (
                        <div key={item.cartId} className="grid grid-cols-[1fr_80px_120px_32px] gap-3 items-center px-3 py-2.5">
                          ...
                          <div className="text-center">
                            {(() => {
                              // For custom items, show Condition from details array
                              if (item.isCustom) {
                                const cond = (item.details || []).find((d) => d.key === "condition");
                                return cond?.value ? (
                              ...
                              // For inventory items, show remark
          ...
          <div className="shrink-0 flex-col-reverse gap-2 border-t border-slate-200 bg-slate-50/80 px-6 py-4 sm:flex-row sm:items-center sm:justify-end sm:space-x-2 sm:px-8">
            <div className="flex items-center gap-2 justify-between w-full sm:w-auto">
              {activeStep > 1 ? (
                ...
                <Button type="button" variant="outline" size="sm" onClick={() => { setForm({ name: "", studentId: "", role: "", expectedReturnAt: "", borrowDate: "" }); setCustomItems([]); setBorrowCart([]); setSelectedInventoryItems([]); inventoryLoadedRef.current = false; setActiveStep(1); }} className="rounded-lg">Cancel</Button>
              )}
              ...
              {activeStep === 2 && (
                <Button type="button" size="sm" onClick={() => {
                  if (borrowCart.length === 0 && customItems.length === 0 && selectedInventoryItems.length === 0) { toast.error("Add at least one item to the cart."); return; }
                  setActiveStep(3);
                }} className={cn("rounded-lg px-5", (borrowCart.length + customItems.length) > 0 || selectedInventoryItems.length > 0 ? "bg-[#4a1111] text-white hover:bg-[#3f0f0f]" : "bg-slate-100 text-slate-400 cursor-not-allowed hover:bg-slate-100")}>Review</Button>
              )}
